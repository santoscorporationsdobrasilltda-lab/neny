...
const placeholderImage = "data:image/svg+xml;base64,";
  ...
  if (error || !product) {
    return (
      <div className="max-w-5xl mx-auto">
        <Link to="/store" className="inline-flex items-center gap-2 text-slate-600 hover:text-purple-600 transition-colors mb-6">
          <ArrowLeft size={16} />
          Voltar
        </Link>
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 glass-effect p-8 rounded-2xl">
                ...
                {product.images.map((_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors ${
                      index === currentImageIndex ? 'bg-purple-500' : 'bg-slate-300 hover:bg-slate-400'
                    }`}
                ...
                {product.images.map((image, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 transition-colors ${
                      index === currentImageIndex ? 'border-purple-500' : 'border-slate-300 hover:border-slate-400'
                    }`}
                  >
                    <img
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            <h1 className="text-4xl font-bold text-slate-800 mb-2">{product.title}</h1>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              {selectedVariant?.sale_price_in_cents && (
                <span className="text-2xl text-slate-400 line-through">{originalPrice}</span>
              )}
            </div>

            <div className="prose text-slate-600 mb-6" dangerouslySetInnerHTML={{ __html: product.description }} />
                ...
                {product.additional_info
                  ...
                  .map((info) => (
                    <div key={info.id} className="border-l-2 border-purple-500/50 pl-4">
                      ...
                      <div className="prose text-slate-600 prose-sm" dangerouslySetInnerHTML={{ __html: info.description }} />
                    </div>
                  ))}
              </div>
            )}
            ...
            {product.variants.length > 1 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-slate-700 mb-2">Estilo</h3>
                <div className="flex flex-wrap gap-2">
                  {product.variants.map(variant => (
                    <Button
                      ...
                      className={`transition-all ${selectedVariant?.id === variant.id ? 'bg-purple-600 border-purple-600 text-white' : 'border-slate-300 text-slate-700 hover:bg-slate-100'}`}
                    >
                      {variant.title}
                    </Button>
                  ))}
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center border border-slate-300 rounded-full p-1">
                <Button onClick={() => handleQuantityChange(-1)} variant="ghost" size="icon" className="rounded-full h-8 w-8 text-slate-600 hover:bg-slate-100"><Minus size={16} /></Button>
                <span className="w-10 text-center text-slate-800 font-bold">{quantity}</span>
                <Button onClick={() => handleQuantityChange(1)} variant="ghost" size="icon" className="rounded-full h-8 w-8 text-slate-600 hover:bg-slate-100"><Plus size={16} /></Button>
              </div>
            </div>
