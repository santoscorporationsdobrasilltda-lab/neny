  ...
  const handleCheckout = useCallback(async () => {
    if (cartItems.length === 0) {
      toast({
        ...
        description: 'Adicione alguns produtos ao seu carrinho antes de finalizar a compra.',
        variant: 'destructive',
      });
      return;
    }
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-2xl font-bold text-slate-800">Carrinho de Compras</h2>
            ...
            <div className="flex-grow p-6 overflow-y-auto space-y-4 custom-scrollbar">
              {cartItems.length === 0 ? (
                ...
                cartItems.map(item => (
                  <div key={item.variant.id} className="flex items-center gap-4 bg-white/50 p-3 rounded-lg">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center border border-slate-200 rounded-md">
                        <Button onClick={() => updateQuantity(item.variant.id, Math.max(1, item.quantity - 1))} size="sm" variant="ghost" className="px-2 text-slate-600 hover:bg-slate-100">-</Button>
                        <span className="px-2 text-slate-800">{item.quantity}</span>
                        <Button onClick={() => updateQuantity(item.variant.id, item.quantity + 1)} size="sm" variant="ghost" className="px-2 text-slate-600 hover:bg-slate-100">+</Button>
                      </div>
                      <Button onClick={() => removeFromCart(item.variant.id)} size="sm" variant="ghost" className="text-red-500 hover:text-red-600 text-xs">Remover</Button>
                    </div>
                  </div>
                ))
              )}
